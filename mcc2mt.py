...
def getcpecustomblock(blockposX, blockposZ, blockposY):
    global CW_BlockID
    CW_BlockID = 0
    if -1 < blockposX < CW_RealWorldSizeX:
        if blockposY < CW_RealWorldSizeY:
            if blockposZ < CW_RealWorldSizeZ:
                CW_BlockID = CW_Blocks_3D[blockposY][blockposZ][blockposX]
...
def CWLoadMap(CCMapFile):
    global CW_Metadata
    global BlockDef
    global CW_WorldFileData
    CW_WorldFile = nbtlib.load(CCMapFile)
    CW_WorldFileData = CW_WorldFile["ClassicWorld"]
    CW_Metadata = CW_WorldFileData["Metadata"]
...
def ConvertBlocks(BlocksModName_input, fileworldname):
    ...
    global CW_CPE
    BlockDef = [[None for y in range(24)] for x in range(768)]
    ...
    if "CPE" in CW_Metadata:
        CW_CPE = CW_Metadata["CPE"]
        print("cpecustom2Minetest: Get BlockDefinitions")
        CW_BlockDefinitions = CW_CPE["BlockDefinitions"]

        for BlockNumber in range(1, 768):
            BlockDefHex = "{:04x}".format(BlockNumber)
            BlockDefName = "Block" + BlockDefHex.upper()
            if BlockDefName in CW_BlockDefinitions:
                CW_Block = CW_BlockDefinitions[BlockDefName]
                ID = CW_Block["ID2"]
                BlockName = CW_Block["Name"]
                CollideType = int(CW_Block["CollideType"])
                TexturesFull = numpy.array(CW_Block["Textures"])
                ...
                TransmitsLight = CW_Block["TransmitsLight"]
                WalkSound = int(CW_Block["WalkSound"])
                FullBright = int(CW_Block["FullBright"])
                Shape = int(CW_Block["Shape"])
                BlockDraw = CW_Block["BlockDraw"]
                FogColor = numpy.array(CW_Block["Fog"])
                FogDensity = int(FogColor[0] % 2**8)
                FogR = int(FogColor[1] % 2**8)
                FogG = int(FogColor[2] % 2**8)
                FogB = int(FogColor[3] % 2**8)
                Coords = numpy.array(CW_Block["Coords"])
        ...
        CW_EnvMapAppearance = CW_CPE["EnvMapAppearance"]
        TextureURL = str(CW_EnvMapAppearance["TextureURL"])
        if TextureURL == "":
            TextureURL = "https://www.cpecustom.net/static/default.zip"
        print(TextureURL)
    else:
        TextureURL = "https://www.cpecustom.net/static/default.zip"

    print("cpecustom2Minetest: Texture: Download")
    ...
    if texturefilefirstbytes == b"PK\x03\x04":
        print("cpecustom2Minetest: Texture: Download: ZIP File")
    ...
    if texturefilefirstbytes == b"\x89PNG":
        print("cpecustom2Minetest: Texture: Download: PNG File")
        os.makedirs("./texture/zip/")
    ...
    # ---------------------------- Crop Textures ----------------------------
    print("cpecustom2Minetest: Texture: Seperate")
    ...
    if not os.path.isdir(
        "./" + fileworldname + "/worldmods/" + BlocksModName + "/textures/"
    ):
    ...
    while CurrentBlockY <= MaxBlockY:
        ...
        BlockImage = BlockTextureImage.crop(
            (BlockCropX, BlockCropY, BlockCropX + BlockSize, BlockCropY + BlockSize)
        )
        BlockImage.save(
            "./"
            + fileworldname
    ...
    dependsfile = open(
        "./" + fileworldname + "/worldmods/" + str(BlocksModName) + "/depends.txt", "w"
    )
    ...
    for TextureNumber in range(0, 512):
        TextureAnim[TextureNumber] = [
            False,
            BlocksModName + str(TextureNumber),
            None,
            None,
            1,
        ]
        ...
        with open(texfolder + "/animations.txt") as animationsfile:
            for line in animationsfile:
                ...
                if "#" not in AnimParams[0] and len(AnimParams) == 7:
                    ...
                    TextureAnim[TextureXYOut] = [
                        True,
                        BlocksModName + str(TextureXYOut) + "_anim",
                        AnimFrameSize,
                        AnimTickDelay,
                        AnimFramesCount,
                    ]

                    AnimFrames = Image.new(
                        "RGBA",
                        (AnimFrameSize, AnimFrameSize * AnimFramesCount),
                        (255, 255, 255, 0),
                    )
                    for AnimFrameCount in range(0, AnimFramesCount):
                        TextureImageFrame = TextureImageAnim.crop(
                            (
                                AnimFrameX + AnimFrameCount * AnimFrameSize,
                                AnimFrameY,
                                AnimFrameX
                                + AnimFrameSize
                                + AnimFrameCount * AnimFrameSize,
                                AnimFrameY + AnimFrameSize,
                            )
                        )
                        AnimFrames.paste(
                            TextureImageFrame, (0, AnimFrameCount * AnimFrameSize)
                        )
                    AnimFrames.save(
                        "./"
                        + fileworldname
    ...
    # ---------------------------- Convert Blocks ----------------------------
    print("cpecustom2Minetest: Minetest Mod: Convert Blocks")
    ...
    initfile = open(
        "./" + fileworldname + "/worldmods/" + BlocksModName + "/init.lua", "w"
    )
    for BlockNumber in range(0, 768):
        ...
        if BlockUsed == 1:
            ...
            if BlockDraw != 4:
                if Shape != 0:
                    if (
                        Coords1 == Coords2 == Coords3 == 0
                        and Coords4 == Coords5 == Coords6 == 16
                    ):
                        ...
                        if (
                            BlockDraw == 2
                        ):  # transparent but with no face culling of same neighbours (e.g. like leaves)
                            initfile.write('\tdrawtype = "nodebox",\n')
                            initfile.write("\tnode_box = {\n")
                            initfile.write('\t\ttype = "fixed",\n')
                            initfile.write(
                        ...
                        if (
                            BlockDraw == 3
                        ):  # translucent, where texture's alpha is blended (e.g. like ice or water)
            ...
            if Shape != 0:
                if BlockName.find("#") == -1:
                    initfile.write(
                        "\ttiles = {"
                        + GetTexture(TextureNum1, "^[transformr180")
                        + ", "
                        + GetTexture(TextureNum2, "")
                        + ", "
                        + GetTexture(TextureNum3, "")
                        + ", "
                        + GetTexture(TextureNum4, "")
                        + ", "
                        + GetTexture(TextureNum6, "")
                        + ", "
                        + GetTexture(TextureNum5, "")
                        + "},\n"
                    )
                else:
                    ...
                    initfile.write(
                        "\ttiles = {"
                        + GetTexture(TextureNum1, "^[transformr180" + tintedblock)
                        + ", "
                        + GetTexture(TextureNum2, tintedblock)
                        + ", "
                        + GetTexture(TextureNum3, tintedblock)
                        + ", "
                        + GetTexture(TextureNum4, tintedblock)
                        + ", "
                        + GetTexture(TextureNum6, tintedblock)
                        + ", "
                        + GetTexture(TextureNum5, tintedblock)
                        + "},\n"
                    )
            else:
            ...
            if Shape != 0:
                ...
                if CollideType == 3:
                    initfile.write(
                        "\tgroups = {cracky = 3, oddly_breakable_by_hand = 3, slippery = 3},\n"
                    )
                if CollideType == 4:
                    initfile.write(
                        "\tgroups = {cracky = 3, oddly_breakable_by_hand = 3, slippery = 5},\n"
                    )
            else:
                initfile.write(
                    "\tgroups = {cracky = 3, oddly_breakable_by_hand = 3},\n"
...
def ConvertEnv(WorldName, fileworldname):
    print("cpecustom2Minetest: Minetest Mod: Convert Env")

    if "CPE" in CW_Metadata:
        CW_EnvColors = CW_CPE["EnvColors"]
        CW_SkyColors = CW_EnvColors["Sky"]
        CW_CloudColors = CW_EnvColors["Cloud"]

        SkyColor_R = int(CW_SkyColors["R"])
        SkyColor_G = int(CW_SkyColors["G"])
        SkyColor_B = int(CW_SkyColors["B"])
        CloudColor_R = int(CW_CloudColors["R"])
        CloudColor_G = int(CW_CloudColors["G"])
        CloudColor_B = int(CW_CloudColors["B"])

        ExtraEnvFound = 0
        CloudsHeight = 0
        CloudsSpeed = 0

        if "EnvMapAspect" in CW_CPE:
            ExtraEnvFound = 1
            CW_EnvMapAspect = CW_CPE["EnvMapAspect"]
            CloudsHeight = int(CW_EnvMapAspect["CloudsHeight"])
            CloudsSpeed = int(CW_EnvMapAspect["CloudsSpeed"])
        ...
        if not os.path.isdir(
            "./" + fileworldname + "/worldmods/" + WorldName + "/textures/"
        ):
            os.makedirs("./" + fileworldname + "/worldmods/" + WorldName + "/textures/")

        initfile = open(
            "./" + fileworldname + "/worldmods/" + WorldName + "/init.lua", "w"
        )
        ...
        if os.path.isfile("./texture/res/skybox.png"):
            ...
            skyboxpart = skyboximage.crop(
                (skyboxsize4 * 1, skyboxsize4 * 0, skyboxsize4 * 2, 0 + skyboxsize4)
            )
            skyboxpart.save(
                "./"
                + fileworldname
            ...
            skyboxpart = skyboximage.crop(
                (skyboxsize4 * 2, skyboxsize4 * 0, skyboxsize4 * 3, 0 + skyboxsize4)
            )
            skyboxpart.save(
                "./"
                + fileworldname
            ...
            skyboxpart = skyboximage.crop(
                (skyboxsize4 * 0, skyboxsize4 * 1, skyboxsize4 * 1, skyboxsize4 * 2)
            )
            skyboxpart.save(
                "./"
                + fileworldname
            ...
            skyboxpart = skyboximage.crop(
                (skyboxsize4 * 1, skyboxsize4 * 1, skyboxsize4 * 2, skyboxsize4 * 2)
            )
            skyboxpart.save(
                "./"
                + fileworldname
            ...
            skyboxpart = skyboximage.crop(
                (skyboxsize4 * 2, skyboxsize4 * 1, skyboxsize4 * 3, skyboxsize4 * 2)
            )
            skyboxpart.save(
                "./"
                + fileworldname
            ...
            skyboxpart = skyboximage.crop(
                (skyboxsize4 * 3, skyboxsize4 * 1, skyboxsize4 * 4, skyboxsize4 * 2)
            )
            skyboxpart.save(
                "./"
                + fileworldname
            ...
            initfile.write(
                '\t\ttextures = {"skybox1.png^[transformR90", "skybox2.png^[transformFXR90", "skybox3.png", "skybox5.png", "skybox4.png", "skybox6.png"},\n'
            )
...
def ConvertWorld(
    BlocksModName, fileworldname, MTChunkPosX, MTChunkPosY, MTChunkPosZ, IsTest
):
    global CW_RealWorldSizeX
    global CW_RealWorldSizeY
    global CW_RealWorldSizeZ
    global CW_Blocks_3D
    CW_RealWorldSizeX = int(CW_WorldFileData["X"])
    CW_RealWorldSizeY = int(CW_WorldFileData["Y"])
    CW_RealWorldSizeZ = int(CW_WorldFileData["Z"])

    if "BlockArray2" in CW_WorldFileData:
        CW_Blocks = (
            numpy.array(CW_WorldFileData["BlockArray"]) % 2**8
            + numpy.array(CW_WorldFileData["BlockArray2"]) * 256
        )
    else:
        CW_Blocks = numpy.array(CW_WorldFileData["BlockArray"]) % 2**8
    CW_Blocks_3D = CW_Blocks.reshape(
        (CW_RealWorldSizeY, CW_RealWorldSizeZ, CW_RealWorldSizeX)
    )
    print(
        "World Size:"
        + str(CW_RealWorldSizeX)
        + " "
        + str(CW_RealWorldSizeY)
        + " "
        + str(CW_RealWorldSizeZ)
    )

    MT_WorldSizeX = int(round_down(CW_RealWorldSizeX / 16))
    MT_WorldSizeY = int(round_down(CW_RealWorldSizeY / 16))
    MT_WorldSizeZ = int(round_down(CW_RealWorldSizeZ / 16))
    ...
    while ConversionComplete == 0:
        print(
            str(MT_CurrentChunkX)
            + " "
            + str(MT_CurrentChunkY)
            + " "
            + str(MT_CurrentChunkZ)
            + " / "
            + str(MT_WorldSizeX)
            + " "
            + str(MT_WorldSizeY)
        ...
        CW_X_BlockPosition = 0
        CW_Y_BlockPosition = 0
        CW_Z_BlockPosition = 0

        MT_BlocksList = []

        while CW_Z_BlockPosition <= 15:

            CW_X_RealBlockPosition = CW_X_BlockPosition * -1 + 15
            CWMT_X_BlockPosition = CW_X_RealBlockPosition + MT_CurrentChunkX * 16
            CWMT_Y_BlockPosition = CW_Y_BlockPosition + MT_CurrentChunkY * 16
            CWMT_Z_BlockPosition = CW_Z_BlockPosition + MT_CurrentChunkZ * 16
            global CW_BlockID
            if IsTest == True:
                CW_BlockID = 1
            else:
                getcpecustomblock(
                    CWMT_X_BlockPosition, CWMT_Z_BlockPosition, CWMT_Y_BlockPosition
                )
            # print(str(CWMT_X_BlockPosition) + ' ' + str(CWMT_Y_BlockPosition) + ' ' + str(CWMT_Z_BlockPosition) + ' ' + str(CW_BlockID))
            MT_BlocksList.append(CW_BlockID)
            if 15 == CW_X_BlockPosition:
                CW_X_BlockPosition = 0
                if 15 == CW_Y_BlockPosition:
                    CW_Y_BlockPosition = 0
                    if 16 != CW_Z_BlockPosition:
                        CW_Z_BlockPosition = CW_Z_BlockPosition + 1
                else:
                    CW_Y_BlockPosition = CW_Y_BlockPosition + 1
            else:
                CW_X_BlockPosition = CW_X_BlockPosition + 1
            writeU16(zlibnodedata, CW_BlockID)
        ...
        MT_Pos = getBlockAsInteger(
            MT_RealCurrentChunkX + MTChunkPosX,
            MT_CurrentChunkY + MTChunkPosY,
            MT_CurrentChunkZ + MTChunkPosZ,
        )
        mapsqlfilecur.execute(
            "INSERT INTO blocks VALUES (?,?)", (int(MT_Pos), mapblockdata.getvalue())
        )
...
def GetSpawnData():
    return [CW_WorldFileData["Spawn"], CW_RealWorldSizeX]


def MT_MakePlayersFile(fileworldname, cwX, cwY, cwZ, cwWX):
    CW_WorldSpawn = CW_WorldFileData["Spawn"]
    CW_SpawnX = int(cwX)
    CW_SpawnY = int(cwY)
    CW_SpawnZ = int(cwZ)

    MT_SpawnX = CW_SpawnX * -1 + cwWX

    playersfile = sqlite3.connect("./" + fileworldname + "/players.sqlite")
    playersfile_cur = playersfile.cursor()

    playersfile_cur.execute(
    ...
    playersfile_cur.execute(
        "CREATE TABLE `player_inventories` (   `player` VARCHAR(50) NOT NULL, `inv_id` INT NOT NULL,  `inv_width` INT NOT NULL, `inv_name` TEXT NOT NULL DEFAULT '',  `inv_size` INT NOT NULL,  PRIMARY KEY(player, inv_id),   FOREIGN KEY (`player`) REFERENCES player (`name`) ON DELETE CASCADE )"
    )

    playersfile_cur.execute(
        'INSERT INTO player_inventories VALUES ("singleplayer", 0, 0, "main", 32)'
    )
    playersfile_cur.execute(
        'INSERT INTO player_inventories VALUES ("singleplayer", 1, 0, "craft", 9)'
    )
    playersfile_cur.execute(
        'INSERT INTO player_inventories VALUES ("singleplayer", 2, 0, "craftpreview", 1)'
    )
    playersfile_cur.execute(
        'INSERT INTO player_inventories VALUES ("singleplayer", 3, 0, "craftresult", 1)'
    )
    ...
    for x in range(0, 32):
        playersfile_cur.execute(
            'INSERT INTO player_inventory_items VALUES ("singleplayer", 0, '
            + str(x)
            + ', " ")'
        )
    ...
    for x in range(0, 9):
        playersfile_cur.execute(
            'INSERT INTO player_inventory_items VALUES ("singleplayer", 1, '
            + str(x)
            + ', " ")'
        )

    playersfile_cur.execute(
        'INSERT INTO player_inventory_items VALUES ("singleplayer", 2, 0, "")'
    )
    playersfile_cur.execute(
        'INSERT INTO player_inventory_items VALUES ("singleplayer", 3, 0, "")'
    )
    ...
    spawncmd = (
        'INSERT INTO player VALUES ("singleplayer", 60, 0, '
        + str(MT_SpawnX * 10)
        + ", "
        + str(CW_SpawnY * 10)
        + ", "
        + str(CW_SpawnZ * 10)
        + ", "
        + "16, 10, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)"
    )
